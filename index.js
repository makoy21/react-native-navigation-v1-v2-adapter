...
Navigation.startTabBasedApp = ({ tabs, tabsStyle, appStyle, drawer }) => {
  const onAppLaunched = () => {
    ...
    Navigation.setDefaultOptions(
      optionsConverter.convertStyle({ ...tabsStyle, ...appStyle }),
    );
...
Navigation.startSingleScreenApp = ({
  ...
  const onAppLaunched = () => {
    ...
    Navigation.setRoot({
      root: layoutGenerator.convertSingleScreen(screen, drawer, components),
    });
  };
  ...
  const Wrapped = class extends React.Component {
    ...
    _isRegisteredToNavigatorEvents() {
      return (
        ...
        this.props.navigator.eventListeners.length > 0
      );
    }

    render() {
      const Component = store
        ? wrapReduxComponent(generator, store, provider)
        : generator();
      return <Component ref={r => (this.ref = r)} {...this.props} />;
    }
  };
